...
class OrderAlertCheck {
    private static final Logger logger = LoggerFactory.getLogger("OrderAlertCheck");
    ...
    String makeMiddleRiskLevelKey(String orderItemSku) {
        return "cnt:sku_attach_order_middle_" + orderItemSku;
    }

    String makeLowRiskLevelKey(String orderItemSku) {
        return "cnt:sku_attach_order_low_" + orderItemSku;
    }

    private void countMiddleOrder(String orderItemSku, String orderid, Long purchasePrice, Long payPrice, Long quantity, Long timestamp) {
        if (null == timestamp) {
            return;
        }
        // 最长5分钟
        int slidwidth = 60 * 5;
        String key = makeMiddleRiskLevelKey(orderItemSku)
    ...
    private void countLowOrder(String orderItemSku, String orderid, Long purchasePrice, Long payPrice, Long quantity, Long timestamp) {
        if (null == timestamp) {
            return;
        }
        // 最长分钟
        int slidwidth = 60 * 10;
        String key = makeLowRiskLevelKey(orderItemSku)
        SmoothSlidingWindowCounterUtil.add(key, orderid, null == timestamp ? System.currentTimeMillis() : timestamp, slidwidth);
        int a = SmoothSlidingWindowCounterUtil.count(key, slidwidth);
    ...
    private void handleBussinessType(final JSONObject jsonObject) {
        ...
        if ("42314".equals(popBusinessNo)) {
            throw new FailfastException(null, "京东商品，跳过")
        }
    }
    ...
    private boolean handleRiskLogicByCounter(final String key, final String orderItemSku, final int slidwidth, final int threshold, List<Pair<String, Object>> alertContext) {
        ...
        if (cnt > threshold) {
            ...
            logger.info("middleRisk sku:{},slidwidth:{},quantity:{}", orderItemSku, slidwidth, skuInt);
            if (skuInt > threshold) {
                alertContext.add(Pair.of("sku", orderItemSku))
    ...
    // 1分钟内，影响商品数量大于等于30，利润率在 -0.3~-0.6
    private boolean handleMiddleRiskLevelLogic1(final String sku, List<Pair<String, Object>> alertContext) {
        String key = makeMiddleRiskLevelKey(sku)
        boolean ret = handleRiskLogicByCounter(key, sku, 60 * 1, 30, alertContext);
        alertContext.add(Pair.of("统计周期", "最近1分钟"));
        return ret;
    }
    // 5分钟内，影响商品数量大于等于50，利润率在 -0.3~-0.6
    private boolean handleMiddleRiskLevelLogic2(final String sku, List<Pair<String, Object>> alertContext) {
        String key = makeMiddleRiskLevelKey(sku)
        boolean ret = handleRiskLogicByCounter(key, sku, 60 * 5, 50, alertContext);
        alertContext.add(Pair.of("统计周期", "最近5分钟"));
        return ret;
    }

    private boolean handleLowRiskLevelLogic(final String orderItemSku, List<Pair<String, Object>> alertContext) {
        String key = makeLowRiskLevelKey(orderItemSku)
        int slidwidth = 60 * 10
        int threshold = 3000 * 100 // fen
        // sku 关联的订单数 cnt
        long cnt = SmoothSlidingWindowCounterUtil.count(key, slidwidth);
        if (0l == cnt) {
            return false;
        ...
        int totalPay = handleRiskLogicPropertiesSumByCounter(orderItemSku, key, "pay", slidwidth);
        int totalPurchase = handleRiskLogicPropertiesSumByCounter(orderItemSku, key, "purchase", slidwidth);
        int missTotal = totalPurchase - totalPay;
        logger.info("lowRisk sku:{},slidwidth:{},missTotal:{}", orderItemSku, slidwidth, missTotal);
        if (missTotal > threshold) {
            alertContext.add(Pair.of("sku", orderItemSku));
            alertContext.add(Pair.of("sku总量", handleRiskLogicPropertiesSumByCounter(orderItemSku, key, "quantity", slidwidth)));
            alertContext.add(Pair.of("总支付利润率", handleMiddlePayRate(totalPay, totalPurchase) + "%"));
            alertContext.add(Pair.of("亏损总金额", missTotal / 100 + "元"));
            handleAttachBindedOrderContext(key, slidwidth, alertContext);
            alertContext.add(Pair.of("统计周期", "最近10分钟"));
            return true;
        }
        return false
    }
    ...
    Map<String, Object> handle(JSONObject jsonObject) {
        ...
        try {
            ...
            int payRate = handleMiddlePayRate(payPrice, purchasePrice * quantity);
            logger.info("商品售卖成本:rate:{},sku:{},order:{},paypreice{},purchase{},quantity{}", payRate, orderItemSku, orderId, payPrice, purchasePrice * quantity, quantity)
            if (-60 < payRate && payRate < -30) {
                countMiddleOrder(orderItemSku, orderId, purchasePrice, payPrice, quantity, timestamp)
                if (handleMiddleRiskLevelLogic1(orderItemSku, alertContext) || handleMiddleRiskLevelLogic2(orderItemSku, alertContext)) {
                    // 命中
                    retData = DecisionResultUtil.wrapAlert(alertContext);
                }
            } else if (-30 <= payRate && payRate < 0) {
                countLowOrder(orderItemSku, orderId, purchasePrice, payPrice, quantity, timestamp)
                if (handleLowRiskLevelLogic(orderItemSku, alertContext)) {
                    // 命中
                    retData = DecisionResultUtil.wrapAlert(alertContext);
                }
            } else {
            ...
            logger.info("云超:非京东，跳过检查:{}", skipException.getMessage())
