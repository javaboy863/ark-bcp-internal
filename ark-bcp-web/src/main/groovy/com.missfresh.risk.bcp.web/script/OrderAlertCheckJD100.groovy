...
class OrderAlertCheckJD100 {
    private static final Logger logger = LoggerFactory.getLogger("OrderAlertCheckJD100");
    int threshold = 100;
    int slidwidth = 60;
    ...
    Map<String, Object> handle(JSONObject jsonObject) {
        ...
        try {
            ...
            if (null == orderType || orderType != 80 || StringUtils.isEmpty(popBusinessNo)) {
                logger.info("OrderAlertCheckJD100.retData1:{}", JSON.toJSONString(retData))
                return retData;
            }
            // 看用户是否是白名单用户
            ...
            if(isIgnoredSku(orderItemSku)) {
                ...
                logger.info("OrderAlertCheckJD100.retData2:{}", JSON.toJSONString(retData))
                return retData;
            }
            ...
            if (null != userphone && !userphone.isEmpty()) {
                if (isBussinissPartener(userphone)) {
                    ...
                    logger.info("OrderAlertCheckJD100.retData3:{}", JSON.toJSONString(retData))
                    return retData;
                }
            }
            ...
            if (!"42314".equals(popBusinessNo)) {
                logger.info("OrderAlertCheckJD100.retData4:{}", JSON.toJSONString(retData))
                return retData;
            }

            if (StringUtils.is(orderId) || StringUtils.is(orderItemSku)) {
                logger.info("OrderAlertCheckJD100.retData5:{}", JSON.toJSONString(retData))
                return retData;
            }
            // 优先积累商品数
            ...
            if (0l == cnt) {
                logger.info("OrderAlertCheckJD100.retData6:{}", JSON.toJSONString(retData))
                return retData;
            }
            if (cnt > threshold) {
                ...
                logger.info("OrderAlertCheckJD100.retData7:{}", JSON.toJSONString(retData))
                return retData;
            } else {
                List<String> orders = SmoothSlidingWindowCounterUtil.members(key, slidwidth);
                if (null == orders || 0 == orders.size()) {
                    logger.info("OrderAlertCheckJD100.retData8:{}", JSON.toJSONString(retData))
                    return retData;
                }
                int skuInt = 0;
                ...
                logger.info("云超:京东，1分钟内，sku售卖数:{},sku:{}", skuInt, orderItemSku);
                if (skuInt > threshold) {
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("sku: ").append(orderItemSku).append("\n");
                    ...
                    retData.put("msg", stringBuilder.toString())
                    retData.put("hited", true)
                    logger.info("OrderAlertCheckJD100.retData9:{}", JSON.toJSONString(retData))
                    return retData;
                }
            }
        } catch (Exception e) {
        ...
        logger.info("OrderAlertCheckJD100.retData10:{}", JSON.toJSONString(retData))
        return retData;
    }
}
